...
Python files can be run top-to-bottom as scripts. Use the guard below to avoid running code when a file is imported as a module:
...
// Rest parameters (equivalent to *args)
const total = (...numbers) => numbers.reduce((acc, n) => acc + n, 0);

// Object spread (equivalent to **kwargs)
const describe = (info) => Object.entries(info).forEach(([k, v]) => console.log(`  ${k}: ${v}`));
