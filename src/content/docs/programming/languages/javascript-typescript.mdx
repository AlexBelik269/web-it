...
// Parallel
var (user, posts) = await (FetchUserAsync(1), FetchPostsAsync(1)).WhenAll();
// Or:
var results = await Task.WhenAll(FetchUserAsync(1), FetchUserAsync(99));
```
</TabItem>
<TabItem label="Python">
```python
...
CompletableFuture<User> fetchUser(int id) {
    return CompletableFuture.supplyAsync(() -> {
        // HTTP call here
        return httpClient.get("/api/users/" + id, User.class);
    });
}

// Parallel
CompletableFuture<User>      userFuture  = fetchUser(1);
CompletableFuture<List<Post>> postFuture = fetchPosts(1);
...
// Workaround: use integer arithmetic (store cents instead of dollars)
// Or compare within epsilon:
