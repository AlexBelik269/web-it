...
- **Message Size:** Wie gross darf eine Nachricht sein?
...
## 6. Datenkapselung (Data Encapsulation)

### Segmentierung von Nachrichten

Grosse Datenmengen werden vor der Übertragung in kleinere **Segmente** aufgeteilt. Dies hat zwei Hauptvorteile:

1. **Erhöhte Geschwindigkeit:** Grosse Datenmenge können übertragen werden, ohne die Leitung vollständig zu blockieren. Andere Kommunikation kann dazwischen übertragen werden (**Multiplexing**).
...
<Callout type="danger"> 
## Summary Module 3: Protocols and Models
</Callout>

**The Rules** — Participants (sender, receiver, media) must agree on how to communicate. Protocols must account for identified sender and receiver, language and grammar, speed and timing of delivery, confirmation requirements.

- **Network Protocol Requirements** — Agree on message encoding, message formatting and encapsulation, message size, message timing, message delivery options.
- **Message Encoding** — Converting information into another acceptable form for transmission.
- **Message Formatting and Encapsulation** — When sent, must use a specific format or structure.
- **Message Timing** — Flow Control (rate of data transmission and speed), Response Timeout (time a device waits when it does not hear a reply), Access method (interval of message sending).
- **Message Delivery Options** — Unicast (one to one), Multicast (one to many), Broadcast (one to all, IPv4).

**Protocols** — Define a common set of rules, at hardware and/or software level. Types:

- **Network Communications** — ≥2 devices to communicate over ≥1 networks.
- **Network Security** — Data authentication, data integrity, data encryption.
- **Routing** — Routers exchange route information, compare path information, select best path.
- **Service Discovery** — Automatic detection of devices and services.

**Protocol Interaction** — Networks require the use of several protocols; each protocol has its own function and format.
- `HTTP` — Governs the way a web server and web client interact; defines content and format.
- `TCP` — Manages individual conversations, guaranteed delivery, flow control.
- `IP` — Delivers messages globally.
- `Ethernet` — Delivers messages from NIC to another NIC on the same LAN.

**Network Protocol Functions** — Addressing (identifies sender and receiver), Reliability (guaranteed delivery), Flow Control (ensures data flows at efficient rate), Sequencing (uniquely labels each transmitted segment), Error Detection (checks data for corruption), Application Interface (process-to-process communications between network applications).

**Protocol Suites** — Group of inter-related protocols necessary to perform a communication function.
- **TCP/IP** — Most common, open standard, maintained by IETF. Operates at application, transport, and internet layers (excluding Network Access).
- **OSI** — Maintained by ISO and ITU.
- **AppleTalk** — Apple.
...
**Internet Standards**
- **ISOC** — Promotes the open development and evolution of the internet.
- **IAB** — Responsible for management and development of internet standards.
- **IETF** — Develops, updates, and maintains internet and TCP/IP technologies.
- **IRTF** — Focused on long-term research related to internet and TCP/IP protocols.
- **ICANN** — Coordinates IP address allocation, management of domain names, assignment of other information.
- **IANA** — Oversees and manages IP address allocation, domain name management, protocol identifiers for ICANN.

**Electronic and Communications Standards**
- **IEEE** — Creates standards for power, energy, healthcare, telecommunications, networking.
- **EIA** — Develops standards for electrical wiring, connectors, 19-inch racks.
- **TIA** — Radio equipment, cellular towers, VoIP, satellite communications.
- **ITU-T** — Defines standards for video compression, IPTV, broadband communications (e.g. DSL).

### Reference Models

| Layer | OSI Model | TCP/IP |
|-------|-----------|--------|
...
- Ethernet, WLAN, SONET, SDH → Physical + Data Link / Network Access
- IPv4, IPv6, ICMPv4, ICMPv6 → Network / Internet
- TCP, UDP → Transport / Transport
- HTTP, DNS, DHCP, FTP → Session + Presentation + Application / Application

### Data Encapsulation

**Segmenting Messages** — Breaking messages into smaller units for speed and efficiency.  
**Multiplexing** — Interleaving multiple streams of segmented data.  
**Sequencing** — Numbering data segments to reassemble them later (TCP).  
**Encapsulation** — Protocols add their information to data; top-down process.
...
**De-Encapsulation** — Data is de-encapsulated as it moves **up** the stack.

### Data Access

Data link and network layers use addressing to deliver data from source to destination.
- **Network Layer** — Source and destination IP addresses responsible for delivering the IP packet.
- **Data Link Layer** — Source and destination addresses responsible for delivering a data link frame from one NIC to another NIC on the same network.

**Layer 3 Logical Address** — IP packet contains source and destination IP address (same link or remote). Packet and frame are not changed during transmission (global).

**Data Link Layer Address** — When devices are on the same Ethernet network, the data link frame uses the MAC address (physically embedded) of the destination NIC. If remote, Layer 3 provides Layer 2 with the local default gateway IP address (router address). First segment is `[Destination NIC, Source NIC]`. Packet is not modified when routing, but the frame is changed from endpoint to endpoint.