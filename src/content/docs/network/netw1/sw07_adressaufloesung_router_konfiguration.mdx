...
## Modul 9 – Adressauflösung

### 9.1 MAC-Adresse vs. IP-Adresse
...
ARP-Requests werden als Broadcasts gesendet und von **jedem Gerät im lokalen Netz** empfangen und verarbeitet. In großen Netzwerken oder wenn viele Geräte gleichzeitig hochfahren, kann dies zu einer erheblichen Netzlast führen – man spricht von einem **Broadcast-Sturm**.
...
**Gegenmaßnahmen:** Enterprise-Switches können **Dynamic ARP Inspection (DAI)** einsetzen, das ARP-Pakete gegen eine DHCP-Snooping-Binding-Tabelle prüft.
...
## Modul 10 – Grundlegende Router-Konfiguration

### 10.1 Initiale Router-Konfiguration
...
**Wichtig:** Interfaces sind auf Cisco-Routern standardmäßig **administrativ deaktiviert** (shutdown). Der Befehl `no shutdown` aktiviert das Interface. Zusätzlich muss das Interface physisch verbunden sein (Kabel eingesteckt), damit auch der physische Layer aktiv wird.
...
### 10.3 Default Gateway konfigurieren

#### Default Gateway auf einem Host

Ein **Default Gateway** (Standardgateway) ist die IP-Adresse des Router-Interfaces, das mit dem lokalen Netzwerk verbunden ist. Wenn ein Host Pakete an ein Ziel außerhalb seines eigenen Subnetzes senden möchte, schickt er das Paket an diesen Router – er kennt ja keinen besseren Weg.
...
**Warum braucht ein Switch ein Gateway?** Wenn ein Administrator von einem anderen Netz aus per SSH auf den Switch zugreift, muss der Switch die Antwortpakete zurückschicken können. Ohne Default Gateway weiß er nicht, wohin Pakete für Ziele außerhalb seines lokalen Netzes gesendet werden sollen.
...
- Router-Interfaces sind standardmäßig **deaktiviert** – `no shutdown` ist Pflicht.
- Konfigurationsänderungen gehen verloren, wenn nicht mit `copy running-config startup-config` gespeichert wird.

---

## Neue Befehle und Konzepte dieser Woche
