...
Used when no user is involved — a service authenticating as itself. The following examples fetch an access token using the client credentials grant.
...
# Cache access_token and refresh 60 seconds before expiry
```
</TabItem>
...
// Cache accessToken and refresh before expiresIn seconds
```
</TabItem>
<TabItem label="Java">
```java
...
// Use Jackson or Gson: objectMapper.readTree(response.body())
```
</TabItem>
</Tabs>
