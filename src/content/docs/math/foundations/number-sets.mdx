...
Floating-point types (`float`, `double`) store a *binary* approximation of a decimal value, not the exact number. Since 0.1 and 0.2 cannot be represented exactly in binary (just like 1/3 cannot be written as a finite decimal), their sum accumulates a tiny rounding error. The result is extremely close to 0.3 but not equal — so the equality check evaluates to `false`. This behavior is not a bug in the language; it is a consequence of IEEE 754 double-precision arithmetic and affects every language that uses it.
...
See [Floats & IEEE 754](/math/data-types/floats) for a full breakdown of how binary floating-point works.
