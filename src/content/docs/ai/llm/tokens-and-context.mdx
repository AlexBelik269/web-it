...
```python
# Using tiktoken (OpenAI's tokeniser — pip install tiktoken)
import tiktoken
...
# Anthropic — built-in token counting API
import anthropic
...
```javascript
// Using tiktoken (npm install tiktoken)
import { encoding_for_model } from "tiktoken";

const enc    = encoding_for_model("gpt-4o");
const text   = "Hello, how many tokens is this sentence?";
...
console.log(response.input_tokens);  // e.g. 12
```
</TabItem>
<TabItem label="C#">
```csharp
// Using Microsoft.ML.Tokenizers (dotnet add package Microsoft.ML.Tokenizers)
...
Console.WriteLine($"Token count: {ids.Count}");  // 9

// For Anthropic, use the REST API (no official .NET SDK yet)
// POST https://api.anthropic.com/v1/messages/count_tokens
```
</TabItem>
<TabItem label="Java">
```java
// Using jtokkit (com.knuddels:jtokkit — community tiktoken port)
import com.knuddels.jtokkit.Encodings;
import com.knuddels.jtokkit.api.EncodingType;

var registry = Encodings.newDefaultEncodingRegistry();
var enc      = registry.getEncoding(EncodingType.CL100K_BASE);  // GPT-4 encoding
String text  = "Hello, how many tokens is this sentence?";
var tokens   = enc.encode(text);

System.out.println("Token count: " + tokens.size());  // 9

// Anthropic Java SDK (com.anthropic:anthropic-java — beta)
import com.anthropic.client.Anthropic;
import com.anthropic.models.*;

var client   = Anthropic.fromEnv();
var response = client.messages().countTokens(
    MessageCountTokensParams.builder()
        .model("claude-3-5-sonnet-20241022")
        .addUserMessage("Hello, how many tokens is this?")
        .build()
);
System.out.println(response.inputTokens());  // e.g. 12
```
</TabItem>
</Tabs>
