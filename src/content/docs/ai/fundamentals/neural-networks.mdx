...
Computing an output from an input — each layer's output becomes the next layer's input. The weights shown here are fixed for illustration; in a real network they would be learned during training.
...
```javascript
// Using TensorFlow.js (npm install @tensorflow/tfjs)
import * as tf from '@tensorflow/tfjs';

const W1 = tf.tensor2d([[0.5, -0.2, 0.1], [0.3, 0.8, -0.4]]);
const b1 = tf.tensor1d([0.1, 0.2, -0.1]);
const W2 = tf.tensor2d([[0.7], [-0.3], [0.5]]);
const b2 = tf.tensor1d([0.0]);

function forward(x) {
  const h = tf.relu(tf.add(tf.matMul(x, W1), b1));
  return tf.sigmoid(tf.add(tf.matMul(h, W2), b2));
}

const x = tf.tensor2d([[1.0, 0.5]]);
forward(x).print();  // approximately [[0.72]]
```
</TabItem>
<TabItem label="C#">
```csharp
// Manual implementation (no library needed for illustration)
static float[] Relu(float[] x) =>
    x.Select(v => Math.Max(0f, v)).ToArray();

static float Sigmoid(float x) =>
    1f / (1f + MathF.Exp(-x));

static float[] MatMulVec(float[] vec, float[,] mat, int outSize) {
    var result = new float[outSize];
...
// Weights — 2 inputs → 3 hidden → 1 output
float[,] W1 = {{ 0.5f, -0.2f,  0.1f }, { 0.3f,  0.8f, -0.4f }};
float[]  b1 = { 0.1f, 0.2f, -0.1f };
float[,] W2 = {{ 0.7f }, { -0.3f }, { 0.5f }};
float[]  b2 = { 0.0f };

float[] Forward(float[] x) {
    var h = Relu(MatMulVec(x, W1, 3).Zip(b1, (a, b) => a + b).ToArray());
    return new[] { Sigmoid(MatMulVec(h, W2, 1)[0] + b2[0]) };
}

Console.WriteLine(Forward(new[] { 1.0f, 0.5f })[0]);  // ≈ 0.72
```
</TabItem>
<TabItem label="Java">
```java
static float relu(float x) { return Math.max(0f, x); }
...
float[] forward(float[] x) {
    ...
    float[] h = new float[3];
    for (int j = 0; j < 3; j++) {
        for (int i = 0; i < x.length; i++) h[j] += x[i] * W1[i][j];
        h[j] = relu(h[j] + b1[j]);
    }
    // Output layer
    float y = b2[0];
    for (int j = 0; j < 3; j++) y += h[j] * W2[j][0];
    return new float[]{ sigmoid(y) };
}

System.out.println(forward(new float[]{1.0f, 0.5f})[0]);  // ≈ 0.72
```
</TabItem>
</Tabs>
