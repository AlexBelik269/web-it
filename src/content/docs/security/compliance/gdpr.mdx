...
# Additional fields added only when there is a documented lawful basis
```
</TabItem>
<TabItem label="C#">
```csharp
// ✗ Binding entire request body
...
var user = new User
{
    Email = request.Email,
    Name = request.Name,
    // No phone, birthdate, employer unless explicitly required
};
```
</TabItem>
<TabItem label="Java">
...
// Other fields only with documented lawful basis
```
</TabItem>
</Tabs>
...
async def data_export(current_user=Depends(require_auth)):
    user_id = current_user.id
    profile, orders, events, prefs = await asyncio.gather(
        User.find_by_id(user_id),
        Order.find_all(user_id=user_id),
        AuditLog.find_all(user_id=user_id),
        Preferences.find_by_id(user_id),
    )
    return {
        "profile": profile,
        "orders": orders,
...
<TabItem label="C#">
```csharp
[HttpGet("/api/me/data-export")]
[Authorize]
public async Task<IActionResult> DataExport()
{
    var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
    var (profile, orders, events, prefs) = await (
        _userService.FindByIdAsync(userId),
        _orderService.FindAllByUserAsync(userId),
        _auditService.FindAllByUserAsync(userId),
        _prefsService.FindByUserAsync(userId)
    );
    return Ok(new { profile, orders, activity = events, preferences = prefs,
...
<TabItem label="Java">
```java
@GetMapping("/api/me/data-export")
@PreAuthorize("isAuthenticated()")
public ResponseEntity<DataExportDto> dataExport(@AuthenticationPrincipal UserDetails user) {
    String userId = user.getUsername();
    return ResponseEntity.ok(DataExportDto.builder()
        .profile(userService.findById(userId))
        .orders(orderService.findAllByUser(userId))
        .activity(auditService.findAllByUser(userId))
        .preferences(prefsService.findByUser(userId))
...
async def delete_account(current_user=Depends(require_auth)):
    ...
    await asyncio.gather(
        ...
        Order.retain_for_tax(user_id),
        EventLog.delete(user_id=user_id),
        SearchIndex.remove(user_id),
        CDN.purge_user_uploads(user_id),
        notify_sub_processors(user_id),
    )
    await Session.delete_all(user_id=user_id)
...
public async Task<IActionResult> DeleteAccount()
{
    var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
    await Task.WhenAll(
        _userService.AnonymizeAsync(userId),
        _orderService.RetainForTaxAsync(userId),
        _eventLogService.DeleteByUserAsync(userId),
        _searchService.RemoveUserAsync(userId),
        _cdnService.PurgeUploadsAsync(userId),
        _subProcessorService.NotifyDeletionAsync(userId)
    );
    await _sessionService.DeleteAllAsync(userId);
...
public ResponseEntity<Map<String, String>> deleteAccount(
    ...
    CompletableFuture.allOf(
        userService.anonymizeAsync(userId),
        orderService.retainForTaxAsync(userId),
        eventLogService.deleteByUserAsync(userId),
        searchService.removeUserAsync(userId),
        subProcessorService.notifyDeletionAsync(userId)
    ).join();
    sessionService.deleteAll(userId);
