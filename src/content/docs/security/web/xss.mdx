...
<div style={{ overflowX: 'auto', width: '25%'}}>
```mermaid
flowchart TD
    ...
    UI --> OE["1️⃣ Output Encoding<br/><code>htmlspecialchars()</code> / <code>textContent</code>"]

    OE --> CSP["2️⃣ Content Security Policy (CSP)<br/>Blocks injected scripts without valid nonce"]

    CSP --> HC["3️⃣ HttpOnly Cookies<br/>JavaScript cannot access session tokens"]

    HC --> IV["4️⃣ Input Validation<br/>Reject malformed or unexpected input early"]

    IV --> TT["5️⃣ Trusted Types<br/>Enforces safe DOM injection APIs"]

    TT --> SAFE([🛡️ XSS Attack Prevented])

    %% Styling
    classDef start fill:#1e293b,stroke:#60a5fa,color:#ffffff,stroke-width:2px;
    classDef defense fill:#163d2a,stroke:#3ddc97,color:#ffffff,stroke-width:2px;
    classDef eend fill:#0f172a,stroke:#22c55e,color:#ffffff,stroke-width:3px;

    class UI start;
    class OE,CSP,HC,IV,TT defense;
    class SAFE eend;
