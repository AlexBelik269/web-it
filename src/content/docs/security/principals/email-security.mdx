...
```dns
_dmarc.company.com TXT "v=DMARC1; p=reject; rua=mailto:dmarc-reports@company.com; ruf=mailto:forensics@company.com; pct=100"
```
...
```mermaid
flowchart TD
    E[Incoming email From: ceo@company.com] --> SPF{SPF check\nIs sending IP authorized?}
    SPF -- Pass --> D{DKIM check\nIs signature valid?}
    SPF -- Fail --> DM1{DMARC\nalignment?}
    D -- Pass --> DM2{DMARC\nalignment?}
    D -- Fail / absent --> DM1
    DM1 -- Aligned --> OK[Email passes DMARC]
    DM2 -- Aligned --> OK
    DM1 -- Not aligned --> DMARC{DMARC policy?}
    DMARC -- p=none --> OK2[Deliver, report only]
    DMARC -- p=quarantine --> SPAM[Deliver to spam]
    DMARC -- p=reject --> REJECT[Reject email]
```
